...
function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    dbo.addtoprofiledb(userid,firstName);
...
passport.deserializeUser( async (userid, done) => {
    ...
    let row = await dbo.getuserdata(userid);
    //console.log(row);
...
// PROJECT 2 SERVER CODE STARTS HERE
...
app.get("/week", isAuthenticated, async function (request,response){
  ...
  let result = await dbo.entries(date,activity,userid);
...
app.post('/store', isAuthenticated, async function(request, response, next) {
  ...
  response.json({
    ...
    scalar: data.scalar,
    units: data.units,
    userID: request.user.userData.profileID,
